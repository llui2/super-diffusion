...
df = pd.read_csv('data/ER-ER-N1000-D50-S0-R0.csv')
...
la_t = li_ER(N, ka, 2)
ax.plot(k2, la_t, color=c[2], marker=' ', markersize=1, linestyle='--', linewidth=1, label='$\\frac{1}{2} \\lambda_2^{\\mathrm{ER}}(N,2p^{\\mathcal{A}})$')
...
ax.text(-0.15, 0.85, "(c)", fontsize=20, ha='center', va='center', transform=ax.transAxes)

plt.savefig(f'plots/fig3c-ER-slice.pdf')
